...
| 項目 | 値 | 備考 |
| --- | --- | --- |
| 名前タグ | 任意の文字列 | VPCを識別する名前を登録 |
| IPv4 CIDR ブロック | 10.0.0.0/16 | ※本講座では16ビットより後がホスト部のため。 |
...
| 大項目 | 小項目 | 値 | 備考 |
| --- | --- | --- | --- |
| VPC | VPC　ID | 自分で作成したVPCを選択 |  |
| サブネットの設定 | サブネット名 | 任意の文字列 `cymbal-public-subnet-1a` | 本講座では「VPC ID」+「public or private」+「subnet」+「アベイラビリティゾーンの値」 |
| サブネットの設定 | サブネット名 | 自分で作成したVPCを選択 |  |

### ルーティング設定

-　パブリックサブネットからインターネットに接続できるようにする
- インターネットではルーターがIPアドレスの行き先を管理しているので、ネットワークとネットワークがIPアドレスを通じて接続することができる（ルーティング）
- ルートテーブルの補足：
  - ルートテーブルは「宛先IPアドレス」と「次のルーター（AWSではターゲット）」という書式で設定する
    ...
    | 10.0.2.0/24 | ルーターB |  |
    | 0.0.0.0/0 | ルーターC | デフォルトルートといって、ルートテーブルに登録されているどのアドレスにも一致しない場合の経路 |

- インターネットゲートウェイを未設定の状態だと、10.0.0.0/16（ターゲット：local）のみなので、10.0.0.0/16以外へは接続できない状態
  - パブリックサブネットからインターネットへ接続できるよう、パブリックサブネットに0.0.0.0/0（ターゲット：インターネットゲートウェイ）を設定する
